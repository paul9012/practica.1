...
elseif ($NrPers == 1) {
      $select = "SELECT  minBrut, maxBrut,o_Persoana FROM deduceri";
        $result = mysqli_query($conn, $select);
                if ($result->num_rows>0) {
                        while($row = $result->fetch_assoc()) {
                             if ($salNet>=$row['minBrut'] && $salNet<=$row['maxBrut']) {
        ...
        elseif ($NrPers == 2) {
      $select = "SELECT  minBrut, maxBrut,doua_Persoane FROM deduceri";
        $result = mysqli_query($conn, $select);
                if ($result->num_rows>0) {
                        while($row = $result->fetch_assoc()) {
                             if ($salNet>=$row['minBrut'] && $salNet<=$row['maxBrut']) {
                             $deduceri_personale=$row['doua_Persoane'];
                                                }
                                            }
                                        }
}
    ...
    elseif ($NrPers == 3) {
      $select = "SELECT  minBrut, maxBrut,trei_Persoane FROM deduceri";
        $result = mysqli_query($conn, $select);
                if ($result->num_rows>0) {
                        while($row = $result->fetch_assoc()) {
                             if ($salNet>=$row['minBrut'] && $salNet<=$row['maxBrut']) {
                             $deduceri_personale=$row['trei_Persoane'];
                                                }
                                            }
                                        }
}
...
elseif ($NrPers >= 4) {
      $select = "SELECT  minBrut, maxBrut,patru_Persoane FROM deduceri";
        $result = mysqli_query($conn, $select);
                if ($result->num_rows>0) {
                        while($row = $result->fetch_assoc()) {
                             if ($salNet>=$row['minBrut'] && $salNet<=$row['maxBrut']) {
                             $deduceri_personale=$row['patru_Persoane'];
                                                }
                                            }
                                        }
}
